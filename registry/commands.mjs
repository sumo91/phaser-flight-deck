...
export const CLI_OPTIONS = Object.freeze({
  ...
  script: option(true, 'PATH', 'Playtest script path (run playtest / baseline / visual-test --script): JSON action steps driving to a state'),
  'at-step': option(true, 'N', 'Run the script prefix up to step N before capture/compare (baseline/visual-test --script; default: all steps)'),
...
export const CLI_COMMANDS = Object.freeze({
  ...
  baseline: command('Capture a visual regression baseline screenshot (optionally script-driven to an in-game state)', 'pdeck baseline <name> [project] [--url URL] [--viewport WxH] [--script playtest.json] [--at-step N]', ['json', 'project', 'url', 'viewport', 'script', 'at-step', 'port', 'timeout'], [positional('name', true, '基准名（[A-Za-z0-9_-]）'), positional('project')]),
  'visual-test': command('Compare the current screen against a visual baseline (pixel diff, browser-decoded; optionally script-driven)', 'pdeck visual-test <name> [project] [--url URL] [--tolerance N] [--threshold N] [--viewport WxH] [--script playtest.json] [--at-step N]', ['json', 'project', 'url', 'viewport', 'tolerance', 'threshold', 'script', 'at-step', 'port', 'timeout'], [positional('name', true, '基准名'), positional('project')]),
...
export const PI_FIELDS = Object.freeze({
  ...
  runAction: field('enum', {
    ...
    description: 'pdeck_run action: serve dev server lifecycle; snapshot screenshot; console error collection; probe runtime state via window.__pdeck; watch streamed observation; observe console-style observation with auto server lifecycle; playtest script-driven robot player (press/hold/click/expect-within/collect/store/capture steps on the real UI)',
  }),
  script: field('string', { maxLength: 512, description: 'Playtest script path (run playtest / baseline / visual-test): JSON file with steps [{do:press|hold|wait|click|expect|collect|store|capture, ...}]' }),
  atStep: field('integer', { minimum: 1, maximum: 64, description: 'baseline/visual-test --script: run the script prefix up to step N before capture/compare (default: all steps)' }),
  visualAction: field('enum', {
    values: ['baseline', 'test'],
    description: 'pdeck_visual action: baseline capture a reference screenshot (use script+atStep to baseline an in-game state); test compare current screen against the baseline (pixel diff)',
  }),
